...
function site_identity_func( $atts = array(), $content = null ) {
	global $hcc_options;
	$logo_url = ($hcc_options['logo']['url']) ? $hcc_options['logo']['url'] : get_template_directory_uri(). '/images/logo.png';
	$logo_option = $hcc_options['logo-option'];
	$html = '';
	$atts = shortcode_atts( array(
		'class' => '',
		'container_class' => ''
...
function email_func( $atts = array(), $content = '' ) {	
	global $hcc_options;
	$contact_email = $hcc_options['contact-email'];
	$html = '';	
	$atts = shortcode_atts( array(
		'offset' => 0,
		'index' => 0,
...
function phone_func( $atts = array(), $content = '' ) {
	...
	if ($atts['index']) :
	    ...
	    $html .= preg_replace('/[^0-9]/', '', $hcc_options['contact-phone'][$i]);
	    $html .= '" >';
	    $html .= $hcc_options['contact-phone'][$i];  
	    $html .= '</a>';
	    $html .= '</span>';		
	else :
		if (@$hcc_options['contact-phone']) :
			foreach ($hcc_options['contact-phone'] as $phone) :
				if ($n > $atts['offset']) :
				    $html .= '<span class="phone-number">';
				    $html .= '<a class="phoneToShow" href="tel:';
...
function fax_func( $atts = array(), $content = '' ) {
	...
	if ($atts['index']) :
	    ...
	    $html .= preg_replace('/[^0-9]/', '', $hcc_options['contact-fax'][$i]);
	    $html .= '" >';
	    $html .= $hcc_options['contact-fax'][$i];  
	    $html .= '</a>';
	    $html .= '</span>';		
	else :
...
function business_hour_func( $atts = array(), $content = '' ) {
	$html = '';
	global $hcc_options;
	$contact_hours = $hcc_options['contact-hour'];
	if ($contact_hours){
...
function address_func( $atts = array(), $content = '' ) {
	...
	if ($atts['index']) :
	    ...
	    $html .= '<span class="address-title">'.$hcc_options['contact-address'][$i]['title'].'</span>';
		if ($hcc_options['contact-address'][$i]['map_link']) :
			$html .= '<a class="address-details" href="'.$hcc_options['contact-address'][$i]['map_link'].'" target="_blank">'.$hcc_options['contact-address'][$i]['description'].'</a>';
		else :
			$html .= '<span  class="address-details">'.$hcc_options['contact-address'][$i]['description'].'</span>';
		endif;
	    $html .= '</span>';
	else :
...
function social_menu_fnc( $atts = array(), $content = '' ) {
	...
	global $hcc_options;
	$html = '';
	$contact_social = $hcc_options['contact-social'];
	$contact_address = $hcc_options['contact-address'];
	$atts = shortcode_atts( array(
		'display' => 'inline',
		'title' => 0,
	), $atts, 'social-menu' );
