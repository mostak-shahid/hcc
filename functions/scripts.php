...
function hcc_enqueue_scripts() {
	...
	if ($hcc_options['basic-styling-stylesheet']) {
		wp_register_style( $hcc_options['basic-styling-stylesheet'], get_template_directory_uri() .  '/css/'.$hcc_options['basic-styling-stylesheet'].'.css', array('main.min'));
		wp_enqueue_style( $hcc_options['basic-styling-stylesheet'] );		
	}

}
add_action( 'wp_enqueue_scripts', 'hcc_enqueue_scripts' );
function hcc_admin_enqueue_scripts(){
	wp_register_style( 'font-awesome.min', get_template_directory_uri() . '/fonts/font-awesome-4.7.0/css/font-awesome.min.css' );
...
add_action( 'admin_enqueue_scripts', 'hcc_admin_enqueue_scripts' );
